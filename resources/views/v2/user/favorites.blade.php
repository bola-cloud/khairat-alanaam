...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <div class="text-end">
        <h3 class="fw-bold mb-1">@lang('v2_home.favorites_title')</h3>
        <p class="text-muted mb-0">({{ $wishlists->count() }} @lang('v2_home.products_count'))</p>
    </div>
</div>
    ...
    @foreach($wishlists as $wishlist)
        ...
        <div class="col-md-6 col-lg-4">
            <div class="product-card" style="background:#fff; border: 1px solid var(--border-color); border-radius:12px; padding:15px; transition: 0.3s; position: relative; display: flex; flex-direction: column; height: 100%;">
                ...
                <span style="position: absolute; top: 150px; {{ $lang == 'fr' ? 'left' : 'right' }}: 25px; background: #e32636; color: #fff; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; z-index: 2;">
                    @lang('v2_store.out_of_stock')
                </span>
                @elseif($product->Quantity > 0 && $product->Quantity <= 5)
                <span style="position: absolute; top: 150px; {{ $lang == 'fr' ? 'left' : 'right' }}: 25px; background: #f59e0b; color: #fff; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; z-index: 2;">
                    @lang('v2_store.limited')
                </span>
                @else
                <span style="position: absolute; top: 150px; {{ $lang == 'fr' ? 'left' : 'right' }}: 25px; background: rgba(240,240,240,0.9); color: #555; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; z-index: 2;">
                ...
                <div style="flex-grow: 1; display: flex; flex-direction: column;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 5px;">
                        <div style="padding-left: 10px;">
                            <h3 style="font-size:15px; font-weight: 800; margin:0 0 5px; line-height: 1.4;"><a href="{{ route('front.product_details', $product->en_Product_Slug ?: $product->id) }}" style="color:var(--text-color); text-decoration:none;">{{ $productName }}</a></h3>
                            <p style="margin: 0; color: var(--text-light); font-size: 11px; line-height: 1.4;">{{ Str::limit($productAbout ?: trans('v2_store.no_description'), 40) }}</p>
                        </div>
                        <div style="display: flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 600; flex-shrink: 0; margin-top: 2px;">
                            <span>(4.5)</span>
                ...
                <div style="margin-top: auto; padding-top: 15px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        @if($hasDiscount)
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span style="text-decoration: line-through; color: var(--text-light); font-size: 11px;">{{ number_format($product->Price, 3) }} @lang('v2_layout.currency_omr')</span>
                            </div>
                        @else
                            <div></div>
                        @endif
                        <div style="font-size: 16px; font-weight: 800; display: flex; align-items: center; gap: 5px; color: var(--primary-color);">
                    ...
                    <form action="{{ route('front.cart') }}" method="GET">
                        ...
                        <button type="submit" style="width:100%; padding: 8px; background: var(--primary-color); color:#fff; border:none; border-radius:8px; font-weight: 700; font-size:13px; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px;">
...
@else
<div class="text-center py-5 bg-white border-light shadow-sm" style="border-radius: 12px; border: 1px solid #dee2e6;">
    ...
    <p class="text-muted mb-4">@lang('v2_home.favorites_empty_desc')</p>
    <a href="{{ route('front.store') }}" class="btn text-white px-4 py-2 rounded-3" style="background-color: #d92624;">@lang('v2_home.browse_store')</a>
</div>
@endif
