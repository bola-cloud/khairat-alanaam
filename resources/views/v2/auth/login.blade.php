...
@section('title', 'تسجيل الدخول - خيرات الأنعام')
    ...
    <section class="auth-section" style="padding: 50px 0;">
        <div class="v2-container">
            ...
            <div style="text-align: center; margin-bottom: 30px;">
                ...
                <h2 style="font-weight: 800; font-size: 24px; color: var(--text-color); margin: 0 0 5px;">{{ __('v2_auth.login_title') }}</h2>
                <p style="color: var(--text-light); margin: 0;">{{ __('v2_auth.welcome_back') }}</p>
            </div>
            ...
            <div
                ...
                <form action="{{ route('user.sign.in.post') }}" method="POST">
                    ...
                    <div
                        style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; font-size: 14px;">
                        <label style="display: flex; align-items: center; gap: 5px; color: var(--text-light);">
                            <input type="checkbox" name="remember"> {{ __('v2_auth.remember_me') }}
                        </label>
                    </div>

                    <button type="submit" class="btn-primary"
                        style="width: 100%; padding: 15px; font-size: 16px; margin-bottom: 15px;">{{ __('v2_auth.login_btn') }}</button>
                </form>
                ...
                <a href="{{ route('user.redirect_google') }}"
                    ...
                    {{ __('v2_auth.login_google') }} <i class="fab fa-google" style="color: #4285F4; font-size: 18px;"></i>
                </a>

                <div style="text-align: center; margin-top: 25px; font-size: 14px;">
                    {{ __('v2_auth.no_account') }} <a href="{{ route('user.sign.up') }}"
                        style="color: var(--primary-color); text-decoration: none; font-weight: 600;">{{ __('v2_auth.register_now') }}</a>
                </div>
            </div>
        </div>
    </section>
